	...
	<div class="home_view">
		...
		<div class="card_list">
			<el-collapse-transition v-if="btnAuth('yonghu','首页统计')">
				<el-card v-show="cardTypeList.closeyonghuChartType1" class="card_view">
					<template #header>
						<div class="index_card_head">
							<div class="card_head_title">
								用户
					...
					<el-collapse-transition>
						<div class="card_item" v-show="cardTypeList.hiddenyonghuChartType1">
							<div id="yonghuteshurenqunEchart1" style="width: 100%;height: 400px;"></div>
						</div>
					</el-collapse-transition>
				</el-card>
			</el-collapse-transition>
			<el-collapse-transition v-if="btnAuth('yonghu','首页统计')">
				<el-card v-show="cardTypeList.closeyonghuChartType2" class="card_view">
					<template #header>
						<div class="index_card_head">
							<div class="card_head_title">
								用户
							...
							<div class="card_head_right">
								<el-icon @click="cardTypeClick('hiddenyonghuChartType2')" class="showIcons"
									:class="cardTypeList.hiddenyonghuChartType2?'showIcons1':''">
									<ArrowUpBold />
								</el-icon>
								<el-icon @click="cardTypeClick('closeyonghuChartType2')" class="closeIcons">
									<CloseBold />
								</el-icon>
							</div>
						</div>
					...
					<el-collapse-transition>
						<div class="card_item" v-show="cardTypeList.hiddenyonghuChartType2">
							<div id="yonghunianlingEchart2" style="width: 100%;height: 400px;"></div>
						</div>
					</el-collapse-transition>
				</el-card>
			</el-collapse-transition>
...
<script setup>
	...
	const cardTypeList = ref({
		...
		closeyonghuChartType2: true,
		hiddenyonghuChartType2: true,
	})
	const getCardList = () => {
	...
	const getyonghuChart1 = () => {
		nextTick(()=>{
			var teshurenqunEchart1 = echarts.init(document.getElementById("yonghuteshurenqunEchart1"),'macarons');
			context?.$http({
				...
				for(let i=0;i<res.length;i++){
				    xAxis.push(res[i].teshurenqun);
				    yAxis.push(parseFloat((res[i].total)));
				    pArray.push({
				        value: parseFloat((res[i].total)),
				        name: res[i].teshurenqun
				    })
				}
				var option = {};
                option = {
	...
	const getyonghuChart2 = () => {
		nextTick(()=>{
			var nianlingEchart2 = echarts.init(document.getElementById("yonghunianlingEchart2"),'macarons');
			context?.$http({
